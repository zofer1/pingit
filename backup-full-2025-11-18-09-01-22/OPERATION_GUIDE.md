...
- ✅ **Only PowerShell commands are used** (PowerShell 5.0+ or PowerShell Core)
- ❌ **Linux/Bash commands will NOT work** in the development environment
- ❌ **DO NOT use `grep`, `ls`, `cat`, `tail -f`, etc.** - Use PowerShell equivalents instead
...
| `tail -f file` | `Get-Content file -Tail 10 -Wait` |
| `ps aux` | `Get-Process` |
| `kill PID` | `Stop-Process -Id PID` |

**Note for AI Systems:**
When working with this project in development, always use PowerShell. If a command fails with "command not found", convert it to PowerShell syntax. The production Linux deployment (Raspberry Pi) uses Bash/Linux commands separately in `setup.sh` and systemd services.
