...
def main():
    ...
    # Load webserver configuration to get the URL
    webserver_url = "http://localhost:5000"  # Default webserver URL
    try:
        if Path(webserver_config_path).exists():
            with open(webserver_config_path, 'r') as f:
                ...
                port = webserver_config.get('webserver', {}).get('port', 5000)
                webserver_url = f"http://{host}:{port}"
    except Exception as e:
        print(f"Warning: Could not load webserver config from {webserver_config_path}, using default: {e}")
